...
const LayerLinearGrid = forwardRef(({defaultVisible = false}, ref) => {
  const items = useRef([])
  ...
  const animate = (show, opts) => {
    ...
    const target = show
      ? defaultVisible ? 'start' : 'end'
      : defaultVisible ? 'end' : 'start'
  ...
  const createTimeline = useCallback(() => {
    ...
    tween.current = gsap.timeline({
      paused: true,
      onUpdate () { lastProgress.current = tween.current.progress },
      onComplete () { lastProgress.current = 1 }
    })
  ...
  const tweenRef = useCallback(() => {
    ...
    if (lastState.current) animate(lastState.current.show, lastState.current.opts)
  }, []);
  ...
  return (
    <LayerGrid ref={tweenRef} onItemsChanged={handleOnItemsChange} />
  )
});
